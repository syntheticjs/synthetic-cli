	...
	render: function() {
		...
		fs.writeFile(this.config.cwd+"index.html", _.template(fs.readFileSync(__dirname+"/files/index.html", "utf-8"))({
			...
			if (err) {
				...
				this.log('Successful creation index.html');
			}
		}.bind(this));

		// Create dev.html
		fs.writeFile(this.config.cwd+"dev.html", _.template(fs.readFileSync(__dirname+"/files/dev.html", "utf-8"))({
			component: this.json,
			JSON: JSON
		}), function(err) {
			if (err) {
				...
				this.log('Successful creation dev.html');
			}
		}.bind(this));

		// Create bower.json
		fs.writeFile(this.config.cwd+"bower.json", _.template(fs.readFileSync(__dirname+"/files/bower.json", "utf-8"))({
			component: this.json,
			JSON: JSON
		}), function(err) {
			if (err) {
				...
				this.log('Successful creation bower.json');
			}
		}.bind(this));

		// Create bowerrc
		fs.writeFile(this.config.cwd+".bowerrc", fs.readFileSync(__dirname+"/files/bowerrc", "utf-8"), function(err) {
			if (err) {
				this.warn('Error writing file');
			} else {
				this.log('Successful creation .bowerrc');
			}
		}.bind(this));
